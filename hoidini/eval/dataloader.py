...
class LoadData_GRAB(data.Dataset):
    def __init__(
        self, args, dataset_dir, ds_name="train", dtype=torch.float32, load_on_ram=False
    ):

        super().__init__()
        current_dirpath = os.path.join(dataset_dir, ds_name)

        self.all_seqs = glob.glob(
            os.path.join(current_dirpath, "*.npz"), recursive=True
        )
        self.args = args
        self.data_dict = {}
        ...
        skip = (
            ...
            else (args.skip_val if ds_name.lower() == "val" else 1)
        )
    ...
    def __getitem__(self, idx):
        ...
        if T < self.args.max_frames:
            ...
            items["global_orient_obj"] = torch.cat(
                (to_tensor(items["global_orient_obj"]), global_orient_obj)
            )
            items["transl"] = np.concatenate((items["transl"], transl), axis=0)
            items["transl_obj"] = np.concatenate(
                (items["transl_obj"], transl_obj), axis=0
            )
            ...
            items["bps_obj_glob"] = np.concatenate(
                (items["bps_obj_glob"], bps_obj_glob), axis=0
            )
            items["rhand_verts"] = np.concatenate(
                (items["rhand_verts"], rhand_verts), axis=0
            )
            items["lhand_verts"] = np.concatenate(
                (items["lhand_verts"], lhand_verts), axis=0
            )
            items["rhand_contact"] = np.concatenate(
                (items["rhand_contact"], rhand_contact), axis=0
            )
            items["lhand_contact"] = np.concatenate(
                (items["lhand_contact"], lhand_contact), axis=0
            )
            if "rhand_contact_map" in items.keys():
                items["rhand_contact_map"] = np.concatenate(
                    (items["rhand_contact_map"], rhand_contact_map), axis=0
                )
                items["lhand_contact_map"] = np.concatenate(
                    (items["lhand_contact_map"], lhand_contact_map), axis=0
                )
